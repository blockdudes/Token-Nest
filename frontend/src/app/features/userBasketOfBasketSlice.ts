...
export const getUserTotalBasketOfBasket = createAsyncThunk("getUserTotalBasketOfBasket", async (address: string, { rejectWithValue, getState }) => {
    try {
        ...
        for (const basketOfBasketAddress of totalBasketOfBasket) {
            ...
            for (const basket of getBasketOfBasketData.basketTokens) {
                ...
                const getBasketTokenContract = getBasketContract(basket.addr, "BASKET");
